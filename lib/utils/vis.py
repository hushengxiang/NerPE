...
def save_batch_heatmaps(batch_image, batch_heatmaps, file_name, normalize=True):
    ...
    for i in range(batch_size):
        ...
        for j in range(num_joints):
            colored_heatmap = cv2.applyColorMap(heatmaps[j, :, :], cv2.COLORMAP_JET)
...
def save_debug_images(config, input, meta, pred, pred_heatmap, prefix, gt_heatmap=None):
    ...
    if config.DEBUG.SAVE_BATCH_IMAGES_PRED:
        save_batch_image_with_joints(
            input, pred, meta['joints_vis'],
            '{}_pred.jpg'.format(prefix)
        )    
    if config.DEBUG.SAVE_HEATMAPS_GT and gt_heatmap is not None:
        save_batch_heatmaps(
            input, gt_heatmap, '{}_hm_gt.jpg'.format(prefix)
        )
    if config.DEBUG.SAVE_HEATMAPS_PRED:
        save_batch_heatmaps(
            input, pred_heatmap, '{}_hm_pred.jpg'.format(prefix)
        )